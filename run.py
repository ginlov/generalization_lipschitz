...
def cal_related_terms(args):
    ...
    for num_cluster in num_cluster_list:
        ...
        list_of_rows.append({
            ...
            "robustness": torch.mean(torch.tensor(five_times_robustness)).item() + torch.mean(train_loss).item(),
            "localrob": torch.mean(torch.tensor(five_times_local_robustness)).item() + torch.mean(train_loss).item(),
            "localsen": torch.mean(torch.tensor(five_times_local_sen)).item() + torch.mean(train_loss).item(),
            "localavg": torch.mean(torch.tensor(five_times_theorem_five)).item(),
        })
